...
COMMENT ON TABLE session_tokens IS 'ChatGPT session token 表，存储用于代理转发的 access token';
COMMENT ON COLUMN session_tokens.id IS 'Token 记录唯一标识';
COMMENT ON COLUMN session_tokens.token_value IS 'ChatGPT access token 值（Bearer token）';
COMMENT ON COLUMN session_tokens.status IS 'Token 状态：active=活跃、expired=已过期、revoked=已撤销';
